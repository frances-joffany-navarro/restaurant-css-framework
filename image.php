    ...
    <main class="container-fluid d-grid gap-3">
        ...
        <nav aria-label="image navigation">
            <ul class="pagination justify-content-center">
                <li class="page-item">
                    <a class="page-link" href="image.php?page=<?php if($currentPage - 1 < 0) {echo 1;}else{ echo $currentPage - 1; } ?>" aria-label="Previous">
                    <span aria-hidden="true">&laquo</span>
                    </a>
                </li>
                <?php for ($i = 1; $i <= $total_pages; $i++) {?>
                    <li class="page-item <?php if($currentPage == $i) {echo 'active';}else{ echo ''; } ?>"><a class="page-link" href="image.php?page=<?php echo $i; ?>"><?php echo $i; ?></a>
                <?php }?>
                <li class="page-item">
                    <a class="page-link" href="image.php?page=<?php if($currentPage + 1 > $total_pages) {echo $total_pages;}else{ echo $currentPage + 1; } ?>" aria-label="Next">
